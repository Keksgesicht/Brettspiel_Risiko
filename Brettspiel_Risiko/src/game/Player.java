...
public class Player {
	...
	public void updateStatus() {
		...
		case INIT:
			...
			won1Fight = false; break;
		case FIGHT:
			state = PlayerStatus.END; break;
		case END:
			state = PlayerStatus.WAIT; break;
		case WAIT:
			state = PlayerStatus.INIT; break;
		}
	}
	
	/**
	 * this method should be called after this Player won a fight
	 * gives the player only one AreaCard in one turn
	 */
	public void fightWon() {
		...
		case 0:
			newCard = AreaCard.SOLDIER; break;
		case 1:
			newCard = AreaCard.CAVALIER; break;
		case 2:
			newCard = AreaCard.TANK; break;
		} cards.add(newCard);
	}
	
	/**
	 * @return
	 * if this Player has not the right cards on his hand ==> 0 <p>
	 * if this Player has the right cards on his hand ==> 1 <p>
	...
	public int ultiReady() {
		...
		} if(t == 3 || c == 3 || s == 3) return 1;
		if(t != 0 && c != 0 && s != 0) return 1;
		return 0;
	}
	...
	public void useUlti() {
		...
		for(AreaCard ac : cards) {
			...
			if(t.size() != 3) cards.addAll(t);
			if(c.size() != 3) cards.addAll(c);
			if(s.size() != 3) cards.addAll(s);
		} else {
			t.remove(0); c.remove(0); s.remove(0);
			cards.addAll(t); cards.addAll(c); cards.addAll(s);
		}
	}
